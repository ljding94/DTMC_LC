...
int main(int argc, char const *argv[])
{
    ...
    finfo = "N" + std::string(argv[1]) + "_Ne" + std::string(argv[2]) + "_L" +
            std::string(argv[3]) + "_kar" + std::string(argv[4]) + "_lam" +
            std::string(argv[5]) + "_Kd" + std::string(argv[6]) + "_q" +
            std::string(argv[7]) + "_Cn" + std::string(argv[8]) + "_kargd" +
            std::string(argv[9]);
    dtmc_lc membrane(beta, N, Ne, L, d0, l0, kar, lam, Kd, Kt, Cn, kargd);
    if (argc == 11)
    {
        ...
        membrane.Thermal(0, int(N / (delta_s * delta_s)) + 1, 2, delta_s,
                         delta_theta);
        // membrane.O_MC_measure(5, 1, int(N / (ds * ds)) + 1, ds,
        membrane.O_MC_measure(20, 10, int(N / (delta_s * delta_s)) + 1, delta_s,
    ...
    else if (argc == 10)
    {
        ...
        membrane.Thermal(1000, N / (delta_s * delta_s), 1, delta_s,
                         delta_theta);
        membrane.O_MC_measure(1500, 50, N / (delta_s * delta_s) + 1, delta_s,
                              delta_theta, folder, finfo, bin_num_r);
        membrane.State_write(folder + "/State_" + finfo + ".txt");
